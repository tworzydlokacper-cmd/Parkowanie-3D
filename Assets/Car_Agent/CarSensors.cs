...
public class CarSensors : MonoBehaviour
{
    ...
    void MeasureParkingGap(bool isSideClear)
    {
        ...
        {
            isGapDetected = true; isGapValid = false;
        ...
        else if (!isSideClear && isGapDetected)
        {
            ...
            if (gap >= 5.5f) { isGapValid = true; uiText.text = "Koperta!"; uiText.color = Color.green; }
            else if (gap >= 2.5f) { isGapValid = true; uiText.text = "Prostopadłe!"; uiText.color = Color.cyan; }
            else { isGapValid = false; uiText.text = "Za mała!"; uiText.color = Color.red; }
        }
    }
}